...
do
	i_sub="${i%%_*}" # Identifier (characters before underscore)
	for j in $(ls ../raw_data/dummy_files/*_R1_001.fastq.gz) # Iterate through R2
	do
		j_sub="${j%%_*}" # Identifier (characters before underscore)
